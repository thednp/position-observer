...
import PositionObserver from "../src/index";
  ...
  const styleTip = <T extends Tooltip>(self: T) => {
    // requestAnimationFrame(() => {
      ...
      const { element, tooltip, container, arrow } = self;
      ...
      const leftBoundry = 0;
      const rightBoundry = htmlcw - scrollbarWidth - 1;
      ...
      const {
        ...
        right: elemRectRight,
        top: elemRectTop,
      } = observerEntry ?  observerEntry.boundingClientRect : getBoundingClientRect(element, true);
      const { x, y } = {
        x: elemRectLeft,
        y: elemRectTop,
      };
